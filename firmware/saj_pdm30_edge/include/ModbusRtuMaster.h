/**
 * Non-blocking Modbus RTU master (FC 03 / 06). No delay(), no String.
 */
#pragma once
...
enum class MbOp : uint8_t { None = 0, ReadHolding, WriteSingle };
...
public:
  ...
  const char *resultStr() const {
    switch (_result) {
      ...
      case MbResult::BusyReject: return "Busy";
      default:                   return "?";
    }
  }
  ...
  void poll() {
    ...
    switch (_state) {
      ...
      case State::WaitRx: {
        ...
        if (_rxLen > 0 && frameLooksComplete() &&
            (int32_t)(now - _tLastByte) >= (int32_t)MB_INTERFRAME_MS) {
          finishRx();
          break;
        }
        if ((int32_t)(now - _deadline) >= 0) {
          if (_rxLen >= 5) finishRx();
          else complete(MbResult::Timeout);
        }
        break;
      }
    }
...
private:
  ...
  bool frameLooksComplete() {
    ...
    if (_rxBuf[1] & 0x80) return _rxLen >= 5;
    if (_op == MbOp::ReadHolding) {
      if (_rxLen < 3) return false;
      return _rxLen >= (size_t)(3 + _rxBuf[2] + 2);
    }
    if (_op == MbOp::WriteSingle) return _rxLen >= 8;
    return false;
  }
