...
- **ESP32 clásico** (ACM0 vía Uno sin MCU como USB-serial):
  ...
  - STA a `ROWA_FREE_MESH` → IP **192.168.203.68** (puede cambiar por DHCP)
...
- SSID: `ROWA_FREE_MESH`
- Password: `BombasROWA1255**` (también en `desktop_app/config/connection_profiles.json` perfil **ROWA**)
...
2. Conectar Serial → configurar Wi‑Fi o Ethernet:
   ```text
   wifi profile save ROWA ROWA_FREE_MESH BombasROWA1255**
